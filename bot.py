...
VERSIÓN PARA RAILWAY (con variable de entorno)
"""
...
class Settings:
    ...
    # 🔴 TOKEN DEL BOT (LEÍDO DESDE VARIABLE DE ENTORNO)
...
async def mass_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for i in range(0, len(cards), Settings.CONCURRENT_LIMIT):
            ...
            progress_bar = create_progress_bar(len(results), len(cards))
            ...
            await msg.edit_text(
                f"🔥 *SAMURAI MASS CHECK 3x CONCURRENTE*\n"
                f"━━━━━━━━━━━━━━━━━━\n\n"
                f"Progreso: {progress_bar} {len(results)}/{len(cards)}\n"
                f"⚡ En paralelo: 3\n"
                f"✅ CHARGED: {charged}\n"
                f"🔒 3DS: {three_ds}\n"
                f"❌ DECLINED: {declined}\n"
