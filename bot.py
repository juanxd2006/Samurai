...
Procesa 3 tarjetas en paralelo con proxies y sitios rotativos
VERSIÓN PARA RAILWAY (MISMO CÓDIGO, SOLO TOKEN EN VARIABLE DE ENTORNO)
"""
...
class Settings:
    """Configuración global del bot - VERSIÓN RAILWAY"""
    # 🔴 TOKEN DEL BOT (LEÍDO DESDE VARIABLE DE ENTORNO EN RAILWAY)
    TOKEN = os.environ.get("BOT_TOKEN")
    if not TOKEN:
        raise ValueError("❌ ERROR: BOT_TOKEN no está configurado en Railway. Por favor, agrega la variable de entorno BOT_TOKEN con tu token.")
    
    # API endpoints (SAMURAI AUTO SHOPIFY CHECKER API)
    API_ENDPOINTS = [
...
class SamuraiClassifier:
    ...
    def classify(response_text: str, status_code: int) -> Dict:
        """
        Retorna: {
            "status": "CHARGED" | "3DS" | "DECLINED" | "CAPTCHA" | "UNKNOWN" | "TIMEOUT" | "ERROR",
            "emoji": "✅" | "🔒" | "❌" | "💸" | "🤖" | "❓" | "⏱️" | "⚠️",
            "amount": "$XX.XX" o None
        }
        """
        ...
        # 1️⃣ DETECTAR CHARGED (Thank You con monto - MEJORADO)
        if "thank you" in response_lower:
            # Buscar cualquier número que parezca un monto
            price_match = re.search(r'(\d+\.?\d*)', response_text)
            if price_match:
                try:
                    amount_float = float(price_match.group(1))
                    ...
                    return {
                        "status": "CHARGED",
                        "emoji": "✅",
                        "amount": f"${price_match.group(1)}"
                    }
            else:
                # Si hay "thank you" pero no encontramos monto, igual es charge
                return {
                    "status": "CHARGED",
                    "emoji": "✅",
                    "amount": None
        ...
        # 5️⃣ DETECTAR DECLINED (varias formas)
        decline_patterns = ["declined", "card_declined", "generic_decline", "do not honor"]
        if any(pattern in response_lower for pattern in decline_patterns):
            return {
                "status": "DECLINED",
                "emoji": "❌",
                "amount": None
        ...
        # 6️⃣ DETECTAR ERRORES ESPECÍFICOS (UNKNOWN)
        unknown_patterns = [
            "product id is empty", 
            "del ammount empty", 
            "r4 token empty",
...
class SamuraiChecker:
    ...
    async def check_card(card_data: Dict, site: str, proxy: str, session: aiohttp.ClientSession) -> Dict:
        ...
        try:
            ...
            # Clasificar con SAMURAI (versión corregida)
